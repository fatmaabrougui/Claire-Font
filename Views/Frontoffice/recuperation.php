	...
	if(!empty($_POST['recup_mail'])){
		...
		if(filter_var($recup_mail,FILTER_VALIDATE_EMAIL)){
               ...
               <table width="600px">
                 <tr>
                   <td>
                     ...
                     Ciquez <a href="localhost\projetwebCRUD\views\\recuperation.php?section=code&code='.$recup_code.'">ici</a> pour initialiser ton mot de passe
         ...
         mail($recup_mail, "Récupération de mot de passe - Clairefontaine", $message, $header);
            header("Location:localhost\projetwebCRUD\views\\recuperation.php?section=code");
			}
			else{
...
if(isset($_POST['verif_submit'],$_POST['verif_code'])) {
   if(!empty($_POST['verif_code'])) {
      ...
      if($verif_req == 1) {
         ...
         header('Location:localhost\projetwebCRUD\views\\recuperation.php?section=changemdp');
      } else {
         $error = "Code invalide";
      }
   } else {
         ...
         if(!empty($mdp) AND !empty($mdpc)) {
            if($mdp == $mdpc) {
              ...
              $del_req->execute(array($_SESSION['recup_mail']));
               header('Location:localhost\projetwebCRUD\views\login.php');
            } else {
...
<html>
<head>
	<title>Recuperation de mail</title>
</head>
<body>
